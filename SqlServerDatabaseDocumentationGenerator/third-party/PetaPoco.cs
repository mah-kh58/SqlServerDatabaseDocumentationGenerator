...
namespace PetaPoco
{
	...
	public class Database : IDisposable
	{
		...
		public Database(string connectionStringName)
		{
			...
			{
				var first = GetConnectionStringByIndex(0);
				if (first == null)
					throw new InvalidOperationException("Can't find any connection strings in configuration.");
				var nameProp = first.GetType().GetProperty("Name");
				connectionStringName = nameProp == null ? "" : (string)nameProp.GetValue(first, null);
			}
			...
			if (cs != null)
			{
				var provProp = cs.GetType().GetProperty("ProviderName");
				var provVal = provProp == null ? null : provProp.GetValue(cs, null) as string;
				if (!string.IsNullOrEmpty(provVal))
			...
			var connProp = cs.GetType().GetProperty("ConnectionString");
			_connectionString = connProp == null ? null : connProp.GetValue(cs, null) as string;
			_providerName = providerName;
			CommonConstruct();
		}

		// Reflection-based helpers to access ConfigurationManager.ConnectionStrings without a compile-time dependency
		private static object GetConnectionStringByName(string name)
		{
			Type cfgType = Type.GetType("System.Configuration.ConfigurationManager, System.Configuration")
							?? Type.GetType("System.Configuration.ConfigurationManager, System.Configuration.ConfigurationManager");
			if (cfgType == null)
				return null;
			var connStringsProp = cfgType.GetProperty("ConnectionStrings", BindingFlags.Static | BindingFlags.Public);
			if (connStringsProp == null)
				return null;
			var connStrings = connStringsProp.GetValue(null, null);
			if (connStrings == null)
				return null;
			var itemProp = connStrings.GetType().GetProperty("Item", new Type[] { typeof(string) });
			if (itemProp != null)
				return itemProp.GetValue(connStrings, new object[] { name });
			return null;
		}

		private static object GetConnectionStringByIndex(int index)
		{
			Type cfgType = Type.GetType("System.Configuration.ConfigurationManager, System.Configuration")
							?? Type.GetType("System.Configuration.ConfigurationManager, System.Configuration.ConfigurationManager");
			if (cfgType == null)
			...
			if (connStrings == null)
				return null;
			var itemProp = connStrings.GetType().GetProperty("Item", new Type[] { typeof(int) });
			if (itemProp != null)
				return itemProp.GetValue(connStrings, new object[] { index });
			return null;
		}
